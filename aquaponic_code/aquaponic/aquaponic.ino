...
const long ledOffDuration = 43200000  ; //12H//8 min;//28800000; // Durée de 8 heures pour la LED éteinte (en millisecondes)
const long ledOnDuration = 43200000; //16 min  //57600000; // Durée de 16 heures pour la LED allumée (en millisecondes)

//led2 time control
const int led2Pin = 3;
//unsigned long previousMillisLED = 0;
//const long ledOffDuration = 43200000;//8 min;//28800000; // Durée de 8 heures pour la LED éteinte (en millisecondes)
//const long ledOnDuration = 43200000; //16 min  //57600000; // Durée de 16 heures pour la LED allumée (en millisecondes)
...
void loop() {
  ...
  if (currentMillis - previousMillisLED <= ledOffDuration) {
    // Si moins de 8 heures se sont écoulées, garder la LED éteinte
    analogWrite(led2Pin, 0); // Éteindre la LED
  } else if (currentMillis - previousMillisLED <= ledOffDuration + ledOnDuration) {
    // Si moins de 16 heures au total se sont écoulées, allumer la LED
    analogWrite(led2Pin, 127); // Allumer la LED
  } else {
    // Réinitialiser le temps pour recommencer le cycle 24 heures
    previousMillisLED = currentMillis;
  }
